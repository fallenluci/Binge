...
export default function Island({ activePage, onChangePage, onRefresh }) {
  ...
  const submitFilm = () => { if (!filmName.trim() || !selCatId) return; addFilm(selCatId, filmName.trim()); setFilmName(''); setShowAddFilm(false); onRefresh(); };
  const submitCat = () => { if (!catName.trim()) return; addCategory(catName.trim(), selColor); setCatName(''); setShowAddCat(false); refreshCats(); onRefresh(); };
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', bottom: 30, left: '50%', transform: 'translateX(-50%)', zIndex: 100 }}>
        {searchMode ? (
          <div className="glass" style={{
            borderRadius: 100, height: 58, padding: '0 20px',
            display: 'flex', alignItems: 'center', gap: 10, width: 'min(340px, 85vw)',
          }}>
          ...
          <div className="glass" style={{
            borderRadius: 100, height: 62, padding: '0 8px',
      ...
      {showAddFilm && (
        <>
          ...
          <div className="glass" style={{
            ...
            animation: 'slideUp 0.35s cubic-bezier(0.34,1.26,0.64,1)', borderBottom: 'none',
          }}>
            ...
            <div style={{ borderRadius: 14, overflow: 'hidden', marginBottom: 16, border: '1px solid rgba(255,255,255,0.1)' }}>
              {cats.map(cat => (
                <div key={cat.id} onClick={() => setSelCatId(cat.id)} style={{
                  ...
                  background: selCatId === cat.id ? 'rgba(10,132,255,0.15)' : 'rgba(255,255,255,0.03)',
                }}>
      ...
      {showAddCat && (
        <>
          ...
          <div className="glass" style={{
            ...
            <button onClick={() => setShowAddCat(false)} style={{ width: '100%', padding: 16, borderRadius: 14, border: 'none', background: 'rgba(255,255,255,0.08)', color: 'var(--text)', fontSize: 16, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', marginTop: 10 }}>Отмена</button>
          </div>
        </>
      )}
    </>
