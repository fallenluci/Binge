...
export default function RatingPicker({ film, onRate, onClose, accentColor }) {
  ...
  return (
    <div style={{
      ...
      background: `radial-gradient(ellipse at 50% 40%, ${accentColor}33 0%, rgba(0,0,0,0.95) 65%)`,
      backdropFilter: 'blur(40px)',
    }}>
      <button onClick={onClose} className="glass" style={{ position: 'fixed', top: 60, right: 22, borderRadius: 16, width: 42, height: 42, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--text)', fontSize: 16, border: '1px solid rgba(255,255,255,0.15)' }}>✕</button>

      <div style={{ fontSize: 17, color: 'var(--text-dim)', textAlign: 'center', padding: '0 50px', marginBottom: 32, fontWeight: 500 }}>{film.name}</div>

      <div className="glass" style={{ position: 'relative', width: 140, height: 200, overflow: 'hidden', cursor: 'grab', userSelect: 'none', borderRadius: 40, border: `1.5px solid ${accentColor}66` }}
      ...
      >
        <div style={{ position: 'relative', transform: `translateY(${offset + ITEM_H}px)`, transition: dragging.current ? 'none' : 'transform 0.32s cubic-bezier(0.25,0.46,0.45,0.94)' }}>
          {RATINGS.map(r => {
            ...
            return (
              <div key={r} style={{ height: ITEM_H, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: diff === 0 ? 72 : 40, color: diff === 0 ? accentColor : 'var(--text)', opacity: diff === 0 ? 1 : diff === 1 ? 0.35 : 0.1, letterSpacing: '-2px' }}>{r}</div>
            );
          })}
        </div>
      ...
      <button onClick={() => onRate(selected)} style={{ marginTop: 44, background: accentColor, border: 'none', borderRadius: 100, padding: '16px 60px', color: 'white', fontSize: 18, fontWeight: 700, cursor: 'pointer', boxShadow: `0 8px 30px ${accentColor}66` }}>Оценить</button>
    </div>
  );
}
